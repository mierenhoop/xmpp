...
static void ParseContent(struct xmppParser *p, struct xmppXmlSlice *slc) {
  int r;
  bool stop = false;
  ...
  if (!ParseElement(p)) {
    ...
    /*while (ParseElement(p)) {
      if (!strcmp(p->x.elem, "text"))
        ParseContent(p, &st->failure.text);
...
#undef X
    ...
    while (ParseElement(p)) {
      if (!strcmp(p->x.elem, "body"))
        ParseContent(p, &st->message.body);
      else
        SkipUnknownXml(p);
    }
